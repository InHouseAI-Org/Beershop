-- ============================================
-- PRODUCTION FIX: Distributor Outstanding Trigger
-- Adds opening balance to outstanding calculation
...
-- Create or replace the function with opening balance support
...
COMMENT ON FUNCTION update_distributor_outstanding() IS 'Auto-updates distributor amount_outstanding including opening balance';

-- Create triggers to auto-update outstanding balance
...
BEGIN
  FOR dist IN SELECT id, name, opening_balance FROM distributors LOOP
    -- Calculate correct outstanding including opening balance
    SELECT
