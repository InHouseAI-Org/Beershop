...
const TCSLedgerTab = () => {
  ...
  const fetchTCSLedger = async (startDate = '', endDate = '') => {
    try {
      setLoading(true);
      setError('');
      ...
      const response = await api.get(`/tcs-ledger${queryString ? '?' + queryString : ''}`);
  ...
  const handleApplyDateFilter = () => {
    fetchTCSLedger(dateRange.startDate, dateRange.endDate);
  };
